 ...
 */
class RequestException extends \YaySMTP\Aws3\GuzzleHttp\Exception\TransferException
{
     ...
     */
    public static function wrapException(\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request, \Exception $e)
    {
        return $e instanceof RequestException ? $e : new \YaySMTP\Aws3\GuzzleHttp\Exception\RequestException($e->getMessage(), $request, null, $e);
    }
    /**
    ...
    public static function create(\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request, ?\YaySMTP\Aws3\Psr\Http\Message\ResponseInterface $response = null, ?\Exception $previous = null, array $ctx = [])
    {
        ...
        if ($level === 4) {
            $label = 'Client error';
            $className = \YaySMTP\Aws3\GuzzleHttp\Exception\ClientException::class;
        } elseif ($level === 5) {
            $label = 'Server error';
            $className = \YaySMTP\Aws3\GuzzleHttp\Exception\ServerException::class;
        } else {
     ...
     * @return string|null
     */
    public static function getResponseBodySummary(\YaySMTP\Aws3\Psr\Http\Message\ResponseInterface $response)
    {
