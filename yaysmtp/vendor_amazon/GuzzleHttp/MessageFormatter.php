...
use YaySMTP\Aws3\Psr\Http\Message\MessageInterface;
...
class MessageFormatter
{
    ...
    public function format(\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request, ?\YaySMTP\Aws3\Psr\Http\Message\ResponseInterface $response = null, ?\Exception $error = null)
    {
        ...
        return preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \YaySMTP\Aws3\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \YaySMTP\Aws3\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
    ...
    private function headers(\YaySMTP\Aws3\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
