...
use YaySMTP\Aws3\GuzzleHttp\HandlerStack;
    ...
    public function __invoke(\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (isset($options['on_headers'])) {
            ...
            try {
                ...
                $response = new \YaySMTP\Aws3\GuzzleHttp\Exception\RequestException($msg, $request, $response, $e);
            }
        }
        if (is_callable($response)) {
            $response = call_user_func($response, $request, $options);
        }
        $response = $response instanceof \Exception ? \YaySMTP\Aws3\GuzzleHttp\Promise\rejection_for($response) : \YaySMTP\Aws3\GuzzleHttp\Promise\promise_for($response);
    ...
    public function append()
    {
        foreach (func_get_args() as $value) {
            if ($value instanceof ResponseInterface || $value instanceof \Exception || $value instanceof PromiseInterface || is_callable($value)) {
                ...
                throw new \InvalidArgumentException('Expected a response or ' . 'exception. Found ' . \YaySMTP\Aws3\GuzzleHttp\describe_type($value));
            }
        }
    }
    /**
