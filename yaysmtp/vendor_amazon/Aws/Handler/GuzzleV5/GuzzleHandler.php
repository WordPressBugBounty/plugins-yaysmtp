...
use YaySMTP\Aws3\GuzzleHttp\Event\EndEvent;
use YaySMTP\Aws3\GuzzleHttp\Exception\ConnectException;
use YaySMTP\Aws3\GuzzleHttp\Exception\RequestException;
use YaySMTP\Aws3\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use YaySMTP\Aws3\GuzzleHttp\Promise;
use YaySMTP\Aws3\GuzzleHttp\Psr7\Response as Psr7Response;
use YaySMTP\Aws3\GuzzleHttp\Stream\Stream;
...
class GuzzleHandler
{
    ...
    public function __invoke(\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \YaySMTP\Aws3\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\YaySMTP\Aws3\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof Psr7StreamInterface) {
                        $exception->getResponse()->setBody(\YaySMTP\Aws3\GuzzleHttp\Stream\Stream::factory(file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \YaySMTP\Aws3\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof Psr7StreamInterface ? new \YaySMTP\Aws3\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\YaySMTP\Aws3\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \YaySMTP\Aws3\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \YaySMTP\Aws3\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\YaySMTP\Aws3\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \YaySMTP\Aws3\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \YaySMTP\Aws3\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
