...
class Validator {
   ...
   */
  public function validate($name, \YaySMTP\Aws3\Aws\Api\Shape $shape, array $input) {
  ...
  private function dispatch(\YaySMTP\Aws3\Aws\Api\Shape $shape, $value) {
  ...
  private function check_structure(\YaySMTP\Aws3\Aws\Api\StructureShape $shape, $value) {
  ...
  private function check_list(\YaySMTP\Aws3\Aws\Api\ListShape $shape, $value) {
    if (!is_array($value)) {
      $this->addError('must be an array. Found ' . \YaySMTP\Aws3\Aws\describe_type($value));
      return;
    }
  ...
  private function check_map(\YaySMTP\Aws3\Aws\Api\MapShape $shape, $value) {
    if (!$this->checkAssociativeArray($value)) {
      return;
    }
  ...
  private function check_blob(\YaySMTP\Aws3\Aws\Api\Shape $shape, $value) {
    static $valid = ['string' => true, 'integer' => true, 'double' => true, 'resource' => true];
    $type = gettype($value);
    if (!isset($valid[$type])) {
      if ($type != 'object' || !method_exists($value, '__toString')) {
        $this->addError('must be an fopen resource, a ' . 'YaySMTP\\Aws3\\GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \YaySMTP\Aws3\Aws\describe_type($value));
      }
    }
  }
  private function check_numeric(\YaySMTP\Aws3\Aws\Api\Shape $shape, $value) {
    if (!is_numeric($value)) {
      $this->addError('must be numeric. Found ' . \YaySMTP\Aws3\Aws\describe_type($value));
      return;
    }
    $this->validateRange($shape, $value, "numeric value");
  }
  private function check_boolean(\YaySMTP\Aws3\Aws\Api\Shape $shape, $value) {
    if (!is_bool($value)) {
      $this->addError('must be a boolean. Found ' . \YaySMTP\Aws3\Aws\describe_type($value));
    }
  }
  private function check_string(\YaySMTP\Aws3\Aws\Api\Shape $shape, $value) {
    if ($shape['jsonvalue']) {
      if (!self::canJsonEncode($value)) {
        $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \YaySMTP\Aws3\Aws\describe_type($value));
      }
      return;
    }
    if (!$this->checkCanString($value)) {
      $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \YaySMTP\Aws3\Aws\describe_type($value));
      return;
    }
  ...
  private function validateRange(\YaySMTP\Aws3\Aws\Api\Shape $shape, $length, $descriptor) {
  ...
  private function checkAssociativeArray($value) {
    if (!is_array($value) || isset($value[0])) {
      $this->addError('must be an associative array. Found ' . \YaySMTP\Aws3\Aws\describe_type($value));
      return false;
    }
    return true;
  }
