...
use YaySMTP\Aws3\Aws\Api\Validator;
...
use YaySMTP\Aws3\GuzzleHttp\Psr7\LazyOpenStream;
...
final class Middleware
{
        ...
        return function (callable $handler) use($api, $bodyParameter, $sourceParameter) {
            return function (\YaySMTP\Aws3\Aws\CommandInterface $command, ?\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, $api, $bodyParameter, $sourceParameter) {
                ...
                if ($source !== null && $operation->getInput()->hasMember($bodyParameter)) {
                    $command[$bodyParameter] = new \YaySMTP\Aws3\GuzzleHttp\Psr7\LazyOpenStream($source, 'r');
    ...
    public static function requestBuilder(callable $serializer)
    {
        return function (callable $handler) use($serializer) {
            return function (\YaySMTP\Aws3\Aws\CommandInterface $command) use($serializer, $handler) {
    ...
    public static function signer(callable $credProvider, callable $signatureFunction)
    {
        return function (callable $handler) use($signatureFunction, $credProvider) {
            return function (\YaySMTP\Aws3\Aws\CommandInterface $command, \YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request) use($handler, $signatureFunction, $credProvider) {
                $signer = $signatureFunction($command);
                return $credProvider()->then(function (\YaySMTP\Aws3\Aws\Credentials\CredentialsInterface $creds) use($handler, $command, $signer, $request) {
    ...
    public static function retry(?callable $decider = null, ?callable $delay = null, $stats = false)
    {
        ...
        return function (callable $handler) use($decider, $delay, $stats) {
            return new \YaySMTP\Aws3\Aws\RetryMiddleware($decider, $delay, $handler, $stats);
        };
    }
    /**
    ...
    public static function invocationId()
    {
        return function (callable $handler) {
            return function (\YaySMTP\Aws3\Aws\CommandInterface $command, \YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request) use($handler) {
        ...
        return function (callable $handler) use($history) {
            return function (\YaySMTP\Aws3\Aws\CommandInterface $command, ?\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, $history) {
                ...
                return $handler($command, $request)->then(function ($result) use($history, $ticket) {
                    ...
                    return \YaySMTP\Aws3\GuzzleHttp\Promise\rejection_for($reason);
                });
            };
        };
    }
        ...
        return function (callable $handler) {
            return function (\YaySMTP\Aws3\Aws\CommandInterface $command, ?\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\YaySMTP\Aws3\Aws\ResultInterface $res) use($start) {
                    ...
                    return \YaySMTP\Aws3\GuzzleHttp\Promise\rejection_for($err);
                });
            };
        };
    }
