...
namespace YaySMTP\Aws3\Aws\Multipart;

use YaySMTP\Aws3\Aws\AwsClientInterface as Client;
use YaySMTP\Aws3\Aws\CommandInterface;
use YaySMTP\Aws3\Aws\CommandPool;
use YaySMTP\Aws3\Aws\Exception\AwsException;
use YaySMTP\Aws3\Aws\Exception\MultipartUploadException;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \YaySMTP\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'YaySMTP\\Aws3\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\YaySMTP\Aws3\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \YaySMTP\Aws3\GuzzleHttp\Promise\coroutine(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\YaySMTP\Aws3\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \YaySMTP\Aws3\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\YaySMTP\Aws3\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\YaySMTP\Aws3\Aws\CommandInterface $command, \YaySMTP\Aws3\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        $state = new \YaySMTP\Aws3\Aws\Multipart\UploadState($id);
        ...
        return function (callable $handler) use(&$errors) {
            return function (\YaySMTP\Aws3\Aws\CommandInterface $command, ?\YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\YaySMTP\Aws3\Aws\ResultInterface $result) use($command) {
                    ...
                    return $result;
                }, function (\YaySMTP\Aws3\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \YaySMTP\Aws3\Aws\Result();
                });
            };
        };
    }
